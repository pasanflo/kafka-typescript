...
export function readVarInt(buffer: Buffer): { value: number; length: number } {
  ...
  if (value >= maxValueForBytes / 2) {
    console.warn('Value exceeds half the maximum possible value for varint, treating as signed integer');
    console.warn(`Value: ${value}, Max Value for ${length} bytes: ${maxValueForBytes}`);  
